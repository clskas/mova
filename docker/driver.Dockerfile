...
COPY --from=builder /app/services/driver-service/dist ./dist
COPY --from=builder /app/services/driver-service/node_modules/.prisma ./node_modules/.prisma
COPY --from=builder /app/services/driver-service/prisma ./prisma
COPY scripts/backup-db.sh scripts/migrate-with-backup.sh /app/scripts/
RUN chmod +x /app/scripts/*.sh
ENV NODE_ENV=production APP_ENV=production MOVA_SERVICE=drivers
EXPOSE 3000
