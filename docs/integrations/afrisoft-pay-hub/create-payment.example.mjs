...
const telecom = (process.env.TELECOM || 'OM').toUpperCase();
...
const payload = {
  ...
  phone,
  telecom,
  reference: `${appId}_pay_${uuid}`,
  purpose: 'pay',
  metadata: { demo: true },
  idempotency_key: `${appId}:pay:${phone}:${Math.floor(Date.now() / 300000)}`,
};
