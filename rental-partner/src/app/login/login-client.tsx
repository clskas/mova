...
import {
  ...
  setLastPhone,
  setToken,
  shouldExpectPinAfterLogout,
  clearExpectPinAfterLogout,
} from "@/lib/auth";
...
export function LoginClient({ forcePin = false }: { forcePin?: boolean }) {
  const router = useRouter();
  const [phone, setPhone] = useState("");
  const [code, setCode] = useState("");
  const [pin, setPin] = useState("");
  const [codeSent, setCodeSent] = useState(false);
  const [pinMode, setPinMode] = useState(forcePin);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const wantPin = forcePin || shouldExpectPinAfterLogout() || Boolean(last);
      if (last) {
        setPhone(last);
        if (wantPin) setPinMode(true);
      } else if (forcePin) {
        setPinMode(false);
      }
    })();
  ...
  function useAnotherNumber() {
    ...
    setCode("");
    setError(null);
    if (forcePin) router.replace("/login");
  }
