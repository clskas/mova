...
const NAV = [
  ...
  { href: "/reservations", label: "Réservations", short: "Résas", icon: "📅" },
...
export function PortalShell({
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden">
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-indigo-100 px-3 sm:px-4 py-2 flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          className="md:hidden inline-flex items-center justify-center min-h-10 min-w-10 rounded-xl border border-indigo-100 text-[#1A1A2E]"
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-30 bg-white/95 backdrop-blur border-t border-indigo-100 grid grid-cols-3 gap-x-0.5 gap-y-0 px-1 pt-1 pb-[max(0.25rem,env(safe-area-inset-bottom))]">
        {NAV.map((item) => (
          <Link
            key={item.href}
            href={item.href}
