  ...
  return (
    <section className="rounded-xl border border-gray-100 bg-white p-4 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <button type="button" onClick={loadInitial} className="text-xs text-indigo-600">
          Actualiser
        </button>
      </div>
        ...
        {([
          ...
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-medium ${
              filter === id ? "bg-indigo-600 text-white" : "bg-gray-50 border border-gray-200 text-gray-600"
            }`}
          >
            {label}
          </button>
      ...
      {!loading && hasMore && (
        <button
          ...
          className="w-full rounded-lg border border-gray-200 py-2 text-sm font-medium text-indigo-600 disabled:opacity-50"
        >
