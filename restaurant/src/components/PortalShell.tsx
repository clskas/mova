...
export function PortalShell({ children, restaurantName }: { children: React.ReactNode; restaurantName?: string }) {
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden">
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-orange-100 px-3 sm:px-4 py-2.5 flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          className="md:hidden inline-flex items-center justify-center min-h-11 min-w-11 rounded-xl border border-orange-100 text-[#1A1A2E]"
      ...
      {menuOpen && (
        <div className="md:hidden fixed inset-0 z-40">
          ...
          <aside className="absolute right-0 top-0 h-full w-[min(86vw,20rem)] bg-white shadow-xl flex flex-col pt-[env(safe-area-inset-top)]">
            <div className="px-4 py-4 border-b border-orange-50 flex items-center justify-between">
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-30 bg-white/95 backdrop-blur border-t border-orange-100 grid grid-cols-4 pb-[env(safe-area-inset-bottom)]">
        {BOTTOM.map((item) => (
          <Link
            key={item.href}
            href={item.href}
