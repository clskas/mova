...
describe('buildMobileAppVersionResponse', () => {
  ...
  it('uses defaults when env is empty', () => {
    ...
    expect(payload.passenger.currentVersionCode).toBe(78);
    expect(payload.driver.currentVersionCode).toBe(78);
  ...
  it('raises a stale env below Play floor 78', () => {
    for (const code of ['71', '72', '73', '74', '75', '76', '77']) {
      const payload = buildMobileAppVersionResponse(
        {
  ...
  it('keeps env above the floor', () => {
    const payload = buildMobileAppVersionResponse(
      {
        MOBILE_PASSENGER_VERSION_CODE: '79',
        MOBILE_DRIVER_VERSION_CODE: '80',
      },
      now,
    );
    expect(payload.passenger.currentVersionCode).toBe(79);
