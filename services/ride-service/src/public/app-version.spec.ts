...
describe('buildMobileAppVersionResponse', () => {
  ...
  it('uses defaults when env is empty', () => {
    ...
    expect(payload.passenger.currentVersionCode).toBe(77);
    expect(payload.driver.currentVersionCode).toBe(77);
  ...
  it('raises a stale env below Play floor 77', () => {
    for (const code of ['71', '72', '73', '74', '75', '76']) {
      const payload = buildMobileAppVersionResponse(
        {
  ...
  it('keeps env above the floor', () => {
    const payload = buildMobileAppVersionResponse(
      {
        MOBILE_PASSENGER_VERSION_CODE: '78',
        MOBILE_DRIVER_VERSION_CODE: '79',
      },
      now,
    );
    expect(payload.passenger.currentVersionCode).toBe(78);
    expect(payload.driver.currentVersionCode).toBe(79);
  });
});
