...
describe('buildMobileAppVersionResponse', () => {
  ...
  it('uses defaults when env is empty', () => {
    ...
    expect(payload.passenger.currentVersionCode).toBe(101);
    ...
    const payload = buildMobileAppVersionResponse(
      {
        ...
        MOBILE_PASSENGER_VERSION_CODE: '101',
        MOBILE_DRIVER_VERSION_CODE: '102',
  ...
  it('raises a stale env below Play floor 101', () => {
    for (const code of ['97', '98', '99', '100']) {
      const payload = buildMobileAppVersionResponse(
        {
