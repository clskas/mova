...
describe('buildMobileAppVersionResponse', () => {
  ...
  it('uses defaults when env is empty', () => {
    ...
    expect(payload.passenger.currentVersionCode).toBe(83);
    expect(payload.driver.currentVersionCode).toBe(83);
  ...
  it('raises a stale env below Play floor 83', () => {
    for (const code of ['71', '72', '73', '74', '75', '76', '77', '78', '79', '80', '81', '82']) {
      const payload = buildMobileAppVersionResponse(
        {
  ...
  it('keeps env above the floor', () => {
    const payload = buildMobileAppVersionResponse(
      {
        MOBILE_PASSENGER_VERSION_CODE: '83',
        MOBILE_DRIVER_VERSION_CODE: '84',
      },
      now,
    );
    expect(payload.passenger.currentVersionCode).toBe(83);
